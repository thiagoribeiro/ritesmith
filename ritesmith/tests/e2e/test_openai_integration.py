...
async def test_lua_generation_produces_valid_script(e2e_client):
    ...
    """
    resp = await e2e_client.post("/generate/lua", json={
...
async def test_lua_execution_returns_correct_output(e2e_client):
    ...
    resp = await e2e_client.post("/executions", json={
...
async def test_plan_propose_approve_persist(e2e_client):
    ...
    # 4a. Propose a plan
    resp = await e2e_client.post("/plans", json={
    ...
    resp = await e2e_client.post(f"/plans/{_state.plan_id}/artifacts", json={
        "status": "validated"
    })
    assert resp.status_code == 200, resp.text
    persisted = resp.json()
...
async def test_workflow_generation_produces_valid_trama_structure(e2e_client):
    ...
    """
    resp = await e2e_client.post("/generate/trama-workflow", json={
        "intent": (
...
async def test_workflow_generation_with_branching(e2e_client):
    ...
    errors = validator.validate(content)
    assert errors == [], (
        "Branching workflow failed structural validation:\n"
        + "\n".join(f"  - {e}" for e in errors)
    )
...
"""
    resp = await e2e_client.post("/validate", json={
        "content": script,
...
"""
    ...
    assert result["valid"] is False, (
        "Expected valid=False for os.execute usage, but got valid=True"
    )
...
async def test_shell_generation_is_forbidden(e2e_client):
    ...
    resp = await e2e_client.post("/generate/shell", json={
...
def _assert_polling_loop(content: dict) -> None:
    ...
    assert sleep_nodes[0].get("durationSeconds", 0) > 0, (
        "Sleep node must have durationSeconds > 0"
    )
    ...
    assert errors == [], (
        "Polling loop workflow failed structural validation:\n"
        + "\n".join(f"  - {e}" for e in errors)
    )
