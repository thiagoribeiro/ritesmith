        ...
        [
            {
                ...
                "action": {
                    "mode": "sync",
                    "request": {"url": "http://svc/a", "verb": "POST"},
            ...
            },
            {
                ...
                "action": {
                    "mode": "sync",
                    "request": {"url": "http://svc/b", "verb": "GET"},
                    "successStatusCodes": [200],
                },
                "next": "end",
            },
        ...
        [
            ...
            },
            {
                ...
                "cases": [
                    {
                        "name": "a",
                        "when": {"==": [{"var": "execution.input.type"}, "a"]},
                        "target": "do_a",
                    }
                ],
                "default": "end",
        ...
        [
            {
                ...
                "action": {
                    ...
                    "request": {
                        ...
                        "verb": "POST",
                        "body": {
                            "capability_name": "web.search_semantic",
                            "input": {"query": "test"},
                        },
                    },
                    "successStatusCodes": [200],
        ...
        "nodes": [
            {
                ...
                "action": {
                    "mode": "sync",
                    "request": {"url": "http://price-svc/btc", "verb": "GET"},
                    "successStatusCodes": [200],
                },
                "next": "check",
            },
            {
                ...
                "cases": [
                    {
                        "name": "dropped",
                        "when": {"<=": [{"var": "nodes.fetch.response.body.price"}, 90]},
                        "target": "alert",
                    },
                    {
                        "name": "exhausted",
            ...
            },
            {
                ...
                "action": {
                    "mode": "sync",
                    "request": {"url": "http://alerts/notify", "verb": "POST"},
                    "successStatusCodes": [200],
                },
                "next": "end",
            },
...
def test_validator_v2_sleep_node_missing_duration():
    ...
    errors = v.validate(defn)
    assert any("durationSeconds" in e for e in errors), (
        f"Expected durationSeconds error, got: {errors}"
    )
                            ...
                            "<=": [
                                {
                                    "/": [
                                        {
                                            "var": "nodes.fetch_price.response.body.bpi.USD.rate_float"
                                        },
                                        {"var": "execution.input.last_price"},
                                    ]
                                },
                                0.97,
                            ]
    ...
    "nodes": [
        {
            "id": "s1",
            "kind": "task",
            "next": "ghost",  # references non-existent node
            "action": {
                "mode": "sync",
                "request": {"url": "http://x", "verb": "GET"},
                "successStatusCodes": [200],
...
class MockWorkflowLLMValidOnFirst(LLMProvider):
    ...
    async def analyze_intent(self, goal: str = "", **kwargs):
        return IntentAnalysis(
            ...
            summary=f"Workflow for: {goal[:40]}",
            domain="general",
...
async def test_generate_workflow_valid_first_attempt(wf_client_valid):
    resp = await wf_client_valid.post(
...
async def test_generate_workflow_repair_loop(wf_client_repair, repair_llm):
    resp = await wf_client_repair.post(
        "/generate/trama-workflow",
        json={
