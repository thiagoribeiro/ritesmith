...
async def run_repair_loop(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            log.warning("LLM parse failure %d/3 on attempt %d", consecutive_parse_failures, attempt)
            if consecutive_parse_failures >= 3:
                log.error("unrecoverable: LLM not following JSON schema after 3 parse failures")
