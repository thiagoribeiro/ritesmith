...
async def _auto_execute(artifact_id: str, plan_id: str) -> None:
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            exec_ = await svc.create_execution(CreateExecutionRequest(
                artifact_id=artifact_id,
                plan_id=plan_id,
                input=continuation_input,
            ))
            log.info("auto_execute artifact=%s plan=%s exec_status=%s continuation=%s",
                     artifact_id, plan_id, exec_.status, bool(continuation_input))

            # Transition plan to executing when delegation is waiting on Trama
            if exec_.status in ("waiting", "running"):
                if plan_orm and plan_orm.status == PlanStatus.approved:
                    plan_orm.status = PlanStatus.executing
                    await db.commit()
    except Exception as e:
        log.error("auto_execute failed artifact=%s plan=%s: %s", artifact_id, plan_id, e, exc_info=True)
